...
void slimmer_nuepresel(TString fname)
{
   ...
   TString dirorigin = "/uboone/data/users/kmiller/ntuples/run3b/";
   TString dir = "/uboone/data/users/kmiller/ntuples/run3b/";
...
//   oldtree->SetBranchAddress("weightsReint",&weightsReint);
   ...
   for (auto i : ROOT::TSeqI(nentries)) {
    ...
    /* if (weightsGenie->size() > 0) {
     ...
     if ( (nslice == 1) && (reco_nu_vtx_sce_x>=10)  && (reco_nu_vtx_sce_x<=246) && (reco_nu_vtx_sce_y>=-106) && (reco_nu_vtx_sce_y<=106) && (reco_nu_vtx_sce_z>=10)  && (reco_nu_vtx_sce_z<=1026) && (contained_fraction > 0.9) && (shr_energy_tot_cali > 0.07) ) { 
       newtree->Fill();
     } 
